...
Иногда может пригодиться использовать значение счётчика, сразу после цикла (при этом чтобы он оставался в локальной области видимости, т.е. в конструкции цикла).<br>


| ```5``` ```then if (str[i] == '\0') {}``` | ```5``` ```затем если (str[i] == '\0') {}```
|-|-|
