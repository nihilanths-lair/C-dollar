...
Если же хотите включить рекурсию, используйте инструкцию: ```recursive function call:```, со флагом: ```enable/disable``` или ```рекурсивный вызов функций: разрешить/запретить```, но будьте предельно осторожны, помня о том, что необходимо создать условие для выхода из бесконечного вызова.
