...
<br>
При попытке скомпилировать такой файл, получите семантическую ошибку, о том, что данная конструкция является пустой (без единой инструкции).
На выходе файл: ```0.bccdlr``` для ```VM-C$``` создан не будет><br>
``` ```

В связи с тем, что конструкция была пустая, то и байт-код не был сгенерирован.<br>
<br>
