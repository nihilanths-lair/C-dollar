...
< Тема: ```Создание, переименование и удаление локальных переменных с временем жизни```><br>
< Тема: ```Динамически выделяемая память на этапе компиляции (не путать с временем выполнения)```><br>
< Тема: ```Строковые/текстовые регистры виртуальной машины для работы с динамически выделяемой памятью (организация менеджера памяти)```><br>
< Тема: ```Использование унарного апострофа или пробела, для лучшей читабельности больших чисел```><br>
< Тема: ```Ветвличный цикл```, он же for в C/C++ (for/then if/then```><br>
< Тема: ```Указатель на счётчик цикла```><br>
@ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @
