...
Точка выполнения скрипта начинается с объявления главной функции, вызываемую внешними модулями скриптов (через установку связей) или игрового движка (API). Три способа как можно объявить функцию, на которую будет передаваться управление потоком кода:
