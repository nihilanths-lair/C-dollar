...
Применяется в тех случаях, когда сразу известно значение, к которому необходимо перейти. Это позволяет избавится от лишних проверок каждого значения на совпадение, тем самым - повысив производительность. Метки всегда должны быть уникальными (без дублирования).
