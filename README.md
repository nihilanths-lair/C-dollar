...
< Тема: ```Ветвличный цикл```, он же for в C/C++, файл: ```0.cdlr``` (исходный код на языке ```C$```)>
<br>
| код на английском | код на русском |
|-|-|
...
| ``` 3``` ``` ``` | ``` 3``` ``` ```
| ``` 4``` ```}``` | ``` 4``` ```}```
| ``` 5``` ```$ Optional constructs, i visible even after the cycle is completed``` | ``` 5``` ```$ Необязательные конструкции, знач видно даже после завершения цикла```
...
< Тема: ```Указатель на счётчик цикла```, файл: ```0.cdlr``` (исходный код на языке ```C$```)>
<br>
