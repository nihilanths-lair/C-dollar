...
Файлы для исходных кодов на ```C$``` используют расширение (формат) – `cdlr/sccdlr`, для откомпилированных в байт-код (без использования JIT-компиляции) – `bccdlr`, в машинный/нативный код (AOT-компиляция) – `mccdlr/nccdlr` <br><br>
Точка выполнения скрипта начинается с объявления главной функции.
