...
< Тема: ```Создание, переименование и удаление локальных переменных (когда больше не нужны) с временем жизни```, файл: ```0.cdlr```><br>
< Тема: ```Динамически выделяемая память на этапе компиляции (не путать с временем выполнения)```, файл: ```0.cdlr```><br>
< Тема: ```Строковые/текстовые регистры виртуальной машины для работы с динамически выделяемой памятью (организация менеджера памяти)```, файл: ```0.cdlr```><br>
< Тема: ```Использование унарного апострофа или пробела, для лучшей читабельности больших чисел```, файл: ```0.cdlr```><br>
< Тема: ```Ветвличный цикл```, он же for в C/C++ (for/then if/then, файл: ```0.cdlr```><br>
< Тема: ```Указатель на счётчик цикла```, файл: ```0.cdlr```><br>
@ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @ @
