...
Если же хотите включить рекурсию, используйте инструкцию: ```recursion```, со флагом: ```enabled``` или ```рекурсивный вызов функций: включить```, но будьте предельно осторожны, помня о том, что необходимо создать условие для выхода из бесконечного вызова.
