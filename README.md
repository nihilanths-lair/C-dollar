...
Если же хотите дать возможность компилятору использовать рекурсию, задайте в его конфигурационном файле, в поле: ```recursive function call:``` – значение: ```enable``` или ```рекурсивный вызов функций: разрешить```, но будьте предельно осторожны, помня о том, что необходимо создать условие для выхода из бесконечного вызова. Помните, лучше лишний раз избегать рекурсивные вызовы функций и использовать вместо них циклы.
...
| ```1``` ```declare function 0()```                | ```1``` ```объявить функцию 0()```
| ```2``` ```{```                                   | ```2``` ```{```
| ```3``` ```    # так делать не рекомендуется```       | ```3``` ```    # так делать не рекомендуется```
| ```4``` ```    recursive function call: enable``` | ```4``` ```    рекурсивный вызов функций: разрешить```
| ```5``` ```    call function 0();```              | ```5``` ```    вызов функции 0();```
| ```6``` ```}```                                   | ```6``` ```}```
