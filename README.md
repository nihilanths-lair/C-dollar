...
<br>
В компиляторе блочные скобки используются по умолчанию, но вы при желании можете их отключить, параметр: ```block_brackets``` или ```блочные_скобки``` как раз за это и отвечает, значения: ```do_not_display``` или ```не_отображать``` тогда ваш код станет более элегантным и чистым. Главное после этого быть внимательным к отступам, т.к. без скобок можно запутаться в логике кода.
