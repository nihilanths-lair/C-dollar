...
Если же хотите включить рекурсию, используйте инструкцию: ```recursion```, со флагом: ```enabled```, но будьте предельно осторожны, помня о том, что необходимо создать условие для выхода из бесконечного цикла.
