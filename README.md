...
Файлы для исходных кодов на ```C$``` используют расширение (формат) – `cdlr/sccdlr`, для откомпилированных в байт-код – `bccdlr/pccdlr`, в машинный/нативный код (AOT-компиляция) – `mccdlr/nccdlr`.<br>
Файлы для исходного кода на чистом ```assembly VM-C$``` имеют расширение – `avmcdlr/scavmdlr`.<br><br>
