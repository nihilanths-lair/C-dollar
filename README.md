...
Файлы для исходных кодов на ```C$``` используют расширение (формат) – `sccdlr`, для откомпилированных в байт-код – `bccdlr`, в машинный/нативный код (AOT-компиляция) – `mccdlr/nccdlr` <br><br>
...
| код на английском | код на русском |
|-|-|
| ```   1``` ```byte n = 0xFFFF;``` | ```   1``` ```байт знач = 0xFFFF;```
| ```   2``` ```goto n``` | ```   2``` ```прыгнуть знач```
| ```   3``` ```label 0xAAAA:``` | ```   3``` ```метка 0xAAAA:```
| ```1002``` ```label 0xFFFF:``` | ```1002``` ```метка 0xFFFF:```
