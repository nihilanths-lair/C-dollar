...
Файлы для исходных кодов на ```C$``` используют расширение (формат) – `cdlr/sccdlr`, для откомпилированных в байт-код – `bccdlr/pccdlr`, в машинный/нативный код (AOT-компиляция) – `mccdlr/nccdlr`.
Файлы для исходного кода на чистом ассемблере ```VM-C$``` имеют расширение – `avmcdlr`<br><br>
