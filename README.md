...
<br>
При попытке скомпилировать такой файл, получите семантическую ошибку, о том, что данная конструкция не несёт смысловую нагрузку.
< откомпилированный файл: ```0.bccdlr``` для ```VM-C$``` создан не будет><br>
``` ```
